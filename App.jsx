...
const TYPES = {all:"Tous",flower:"🌿 Fleurs",resin:"🧱 Résines",vape:"💨 Vapes",comestible:"🍪 Comestibles",accessoire:"🛠️ Accessoires"};
const INTENSITIES = {MODERATE:{label:"MODÉRÉ",color:"#22c55e",dots:2},STRONG:{label:"FORT",color:"#f59e0b",dots:3},INTENSE:{label:"INTENSE",color:"#ef4444",dots:4},EXTREME:{label:"EXTRÊME",color:"#dc2626",dots:5}};

const getMolColor = m => (MOLECULES.find(x=>x.id===m)||{}).color||"#666";

function Stars({r,s=12}){return <div style={{display:"flex",gap:1}}>{[1,2,3,4,5].map(i=><span key={i} style={{fontSize:s,color:i<=Math.round(r)?"#eab308":"#333"}}>★</span>)}</div>}
function IntensityDots({intensity}){const info=INTENSITIES[intensity];if(!info)return null;return <div style={{display:"flex",alignItems:"center",gap:5}}>{[1,2,3,4,5].map(i=><div key={i} style={{width:7,height:7,borderRadius:"50%",background:i<=info.dots?info.color:"rgba(255,255,255,0.1)",boxShadow:i<=info.dots?`0 0 5px ${info.color}60`:"none",animation:intensity==="EXTREME"&&i<=info.dots?"blink 1.5s ease-in-out infinite":"none",animationDelay:`${i*.1}s`}}/>)}<span style={{fontSize:9,fontWeight:800,color:info.color,letterSpacing:".08em",marginLeft:3}}>{info.label}</span></div>}

function Img({src,name,style}){const[e,setE]=useState(false);const[l,setL]=useState(false);const c=["#6B21A8","#7C3AED","#22c55e","#f97316","#eab308","#ec4899"][((name||"").length)%6];if(e)return<div style={{...style,background:`linear-gradient(135deg,${c}30,#111)`,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column"}}><span style={{fontSize:36,opacity:.5}}>🔮</span><span style={{fontSize:10,color:"#888",mt:6,fontWeight:600}}>{name}</span></div>;return<div style={{...style,position:"relative",overflow:"hidden"}}>{!l&&<div style={{position:"absolute",inset:0,background:`linear-gradient(135deg,${c}20,#080808)`,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:24,animation:"spin 2s linear infinite"}}>🔮</span></div>}<img src={src} alt={name} onError={()=>setE(true)} onLoad={()=>setL(true)} style={{width:"100%",height:"100%",objectFit:"cover",opacity:l?1:0,transition:"opacity .4s"}} crossOrigin="anonymous" referrerPolicy="no-referrer"/></div>}

function ProductCard({product:p,onClick}){const[h,setH]=useState(false);const fp=Object.values(p.prices)[0];const fk=Object.keys(p.prices)[0];const mc=getMolColor(p.molecule);
...
{p.molecule?(MOLECULES.find(m=>m.id===p.molecule)||{}).emoji||"":"🛠️"} {(p.molecule||"ACCESSOIRE").toUpperCase().replace("-"," ")}
</div>
...
function ProductDetail({product:p,onClose,onAddCart,allProducts}){
const[qty,setQty]=useState(1);const[selW,setSelW]=useState(Object.keys(p.prices)[0]);const[tab,setTab]=useState("desc");
const price=p.prices[selW];const mc=getMolColor(p.molecule);const isVape=p.type==="vape"||p.type==="comestible";
...
<div style={{position:"relative"}}><Img src={p.image} name={p.name} style={{width:"100%",height:420,background:"#070707"}} className="detail-img"/>
<div style={{position:"absolute",top:14,left:14,background:`linear-gradient(135deg,${mc}cc,${mc})`,color:"#fff",padding:"4px 12px",borderRadius:8,fontSize:11,fontWeight:700,boxShadow:`0 0 16px ${mc}50`}}>{(MOLECULES.find(m=>m.id===p.molecule)||{}).emoji||"🛠️"} {(p.molecule||"ACCESSOIRE").toUpperCase().replace("-"," ")}</div>
...
<p style={{fontSize:12,color:"#bbb",lineHeight:1.6,margin:0,fontStyle:"italic"}}>"{p.smokellier}"</p></div>}
{/* TABS */}
<div style={{display:"flex",gap:0,marginBottom:12,borderBottom:"1px solid #1a1a1a"}}>
{[["desc","Description"],["effects","Effets"],["legal","Légal"]].map(([k,v])=><button key={k} onClick={()=>setTab(k)} style={{background:"none",border:"none",borderBottom:tab===k?`2px solid ${mc}`:"2px solid transparent",color:tab===k?"#fff":"#666",padding:"6px 14px",fontSize:11,fontWeight:700,cursor:"pointer",transition:"all .2s"}}>{v}</button>)}
</div>
...
{[["🚚","Livraison 48h","Dès 49€"],["🔒","Paiement sécurisé","CB / Virement"],["📦","Emballage discret","Neutre"]].map(([i,t,s])=><div key={t} style={{textAlign:"center",padding:"6px",background:"rgba(255,255,255,.02)",borderRadius:6}}><div style={{fontSize:14}}>{i}</div><div style={{fontSize:9,color:"#ccc",fontWeight:700}}>{t}</div><div style={{fontSize:8,color:"#555"}}>{s}</div></div>)}</div>
</div></div>
{/* RELATED */}
...
{related.map(r=><div key={r.id} onClick={()=>{onClose();setTimeout(()=>onClick&&0,100);}} style={{background:"rgba(255,255,255,.02)",borderRadius:10,overflow:"hidden",cursor:"pointer",border:"1px solid #1a1a1a"}}>
...
const total=items.reduce((s,i)=>s+i.price*i.qty,0);const shipping=total>=49?0:4.90;
...
{items.length===0?<div style={{textAlign:"center",paddingTop:60}}><div style={{fontSize:48,marginBottom:12,opacity:.3}}>🛒</div><p style={{color:"#555",fontSize:13}}>Panier vide</p><p style={{color:"#444",fontSize:11}}>Ajoute des produits ! 🔥</p></div>
...
const[page,setPage]=useState("home");const[filter,setFilter]=useState("all");const[molFilter,setMolFilter]=useState(null);
const[search,setSearch]=useState("");const[selected,setSelected]=useState(null);const[cart,setCart]=useState([]);
const[showCart,setShowCart]=useState(false);const[notif,setNotif]=useState(null);const[ageOk,setAgeOk]=useState(false);
const[mobileMenu,setMobileMenu]=useState(false);const[showSearch,setShowSearch]=useState(false);
...
if(sortBy==="price-asc")list=[...list].sort((a,b)=>Object.values(a.prices)[0]-Object.values(b.prices)[0]);
if(sortBy==="price-desc")list=[...list].sort((a,b)=>Object.values(b.prices)[0]-Object.values(a.prices)[0]);
if(sortBy==="rating")list=[...list].sort((a,b)=>(b.rating||0)-(a.rating||0));
if(sortBy==="popular")list=[...list].sort((a,b)=>(b.reviews||0)-(a.reviews||0));
return list;},[filter,molFilter,search,sortBy]);

const addToCart=(product,weight,qty)=>{
setCart(prev=>{const idx=prev.findIndex(i=>i.id===product.id&&i.weight===weight);if(idx>=0){const n=[...prev];n[idx]={...n[idx],qty:n[idx].qty+qty};return n;}
return[...prev,{id:product.id,name:product.name,image:product.image,weight,qty,price:product.prices[weight]}];});
setSelected(null);setNotif(`${product.name} ajouté au panier !`);setTimeout(()=>setNotif(null),2500);};
const updateQty=(i,q)=>setCart(prev=>{const n=[...prev];n[i]={...n[i],qty:q};return n;});
const removeFromCart=i=>setCart(prev=>prev.filter((_,idx)=>idx!==i));
const cartCount=cart.reduce((s,i)=>s+i.qty,0);

// AGE GATE
if(!ageOk)return<div style={{minHeight:"100vh",background:"#050505",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans',system-ui,sans-serif"}}>
<style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:ital,opsz,wght@0,9..40,300;0,9..40,500;0,9..40,700;0,9..40,900&display=swap');*{box-sizing:border-box;margin:0;padding:0}body{background:#050505!important}`}</style>
...
return<div style={{minHeight:"100vh",background:"#050505",color:"#fff",fontFamily:"'DM Sans',system-ui,sans-serif"}}>
<style>{`
...
*{box-sizing:border-box;margin:0;padding:0}body{background:#050505!important}
...
@media(max-width:768px){.pgrid{grid-template-columns:repeat(2,1fr)!important;gap:10px!important}.hero-t{font-size:32px!important}.hero-s{font-size:13px!important}.dgrid{grid-template-columns:1fr!important}.dimg{height:250px!important}.nav-i{padding:0 14px!important}.info-g{grid-template-columns:1fr!important}.foot-g{grid-template-columns:1fr 1fr!important;gap:20px!important}}
...
{/* Desktop nav items */}
{["molecules","boutique","about"].map(p=><button key={p} onClick={()=>{setPage(p==="molecules"?"home":p);if(p==="boutique"){setPage("home");}if(p==="molecules"){setPage("home");}}} style={{background:"none",border:"none",color:"#888",fontSize:12,fontWeight:600,cursor:"pointer",padding:"6px 12px",display:window.innerWidth>768?"block":"none"}}>
{p==="molecules"?"🔮 Molécules":p==="boutique"?"🛒 Boutique":"À Propos"}</button>)}
{/* Search toggle */}
...
{/* Mobile menu */}
<button onClick={()=>setMobileMenu(!mobileMenu)} style={{background:"rgba(255,255,255,.04)",border:"none",color:"#fff",borderRadius:8,padding:"7px 10px",fontSize:16,cursor:"pointer",display:"none"}}>☰</button>
</div></div>
{/* Search bar */}
...
<p style={{color:"#999",fontSize:14,lineHeight:1.8,marginBottom:16}}>CannaZen est né d'une conviction : le cannabis légal mérite mieux que ce qui existe. Mieux en qualité, mieux en transparence, mieux en expérience. Chaque produit est sélectionné avec l'exigence d'un sommelier du chanvre — notre Smokellier — qui ne laisse rien passer.</p>
<p style={{color:"#999",fontSize:14,lineHeight:1.8}}>Avec plus de {PRODUCTS.length} produits et {MOLECULES.length} molécules exclusives, nous proposons la gamme la plus complète du marché français. De la fleur CBD douce pour les débutants à la Magic Sauce pour les connaisseurs exigeants, il y a un produit CannaZen pour chaque moment.</p>
</div>
<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:24}} className="info-g">
{[["🔬","Qualité labo","Chaque lot est analysé en laboratoire indépendant. Certificats disponibles sur demande."],
["🌱","Culture premium","Chanvre européen cultivé dans des conditions optimales, sans pesticides."],
["🚚","Livraison express","Expédition sous 24h. Livraison en 48h. Gratuite dès 49€."],
["🔒","100% légal","Tous nos produits respectent la législation (THC < 0.3%). Conformité garantie."]
].map(([i,t,d])=><div key={t} style={{background:"#111",borderRadius:14,padding:24,border:"1px solid #1a1a1a"}}>
<div style={{fontSize:28,marginBottom:10}}>{i}</div>
<h3 style={{fontSize:15,fontWeight:800,color:"#fff",marginBottom:6}}>{t}</h3>
<p style={{fontSize:12,color:"#777",lineHeight:1.6}}>{d}</p></div>)}</div>
<div style={{background:`linear-gradient(135deg,rgba(147,51,234,.1),rgba(147,51,234,.03))`,borderRadius:16,padding:32,border:"1px solid rgba(147,51,234,.15)"}}>
<h2 style={{fontSize:20,fontWeight:800,color:"#fff",marginBottom:12}}>🔮 La Magic Sauce</h2>
<p style={{color:"#999",fontSize:14,lineHeight:1.8,marginBottom:12}}>La Magic Sauce représente le summum de notre expertise. Cette formule exclusive combine un mélange unique de cannabinoïdes soigneusement sélectionnés pour une expérience sensorielle incomparable.</p>
<p style={{color:"#999",fontSize:14,lineHeight:1.8}}>Développée en partenariat avec les meilleurs producteurs européens, elle respecte la législation tout en offrant une puissance et une qualité exceptionnelles. Disponible en fleurs, résines et vapes.</p>
</div>
...
<div><h2 style={{fontSize:24,fontWeight:900,color:"#fff"}}>{m.name}</h2>
<p style={{fontSize:13,color:"#888"}}>{m.desc}</p></div>
</div>
...
<div className="filter-scroll" style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:4,flex:1}}>
...
<p style={{fontSize:13,color:"#888",lineHeight:1.8,marginBottom:12}}>CannaZen sélectionne uniquement des produits d'exception, analysés en laboratoire, cultivés en Europe dans les meilleures conditions. Notre Smokellier goûte et valide chaque variété.</p>
...
{[["🧪","Formule exclusive","Cannabinoïdes optimisés"],["🌱","Culture premium","Sans pesticides"],["📋","Certifié labo","Pureté garantie"],["⚖️","100% légal","THC < 0.3%"]].map(([i,t,d])=>
...
<span style={{fontSize:18}}>🍃</span><span style={{fontSize:16,fontWeight:900}}><span style={{color:"#fff"}}>Canna</span><span style={{color:"#9333EA"}}>Zen</span></span></div>
...
{["📧","📱","💬"].map(i=><div key={i} style={{width:32,height:32,borderRadius:8,background:"rgba(255,255,255,.03)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,cursor:"pointer"}}>{i}</div>)}
</div></div>
...
{[["À Propos",()=>setPage("about")],["CGV",null],["Mentions légales",null],["Confidentialité",null],["Cookies",null]].map(([l,fn])=><div key={l} onClick={fn} style={{fontSize:11,color:"#444",padding:"3px 0",cursor:fn?"pointer":"default"}}>{l}</div>)}</div>
</div>
